...
def init_centroids(num_clusters, image):
    ...
    # *** START YOUR CODE ***
    #initialize centroid
    req_img = image.reshape((image.shape[0] * image.shape[1], 3))
    indices = random.sample(range(req_img.shape[0]), num_clusters)
    centroids_init = req_img[indices,:]
    # *** END YOUR CODE ***
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    req_img = image.reshape((image.shape[0] * image.shape[1], 3))
    z = np.empty((req_img.shape[0],1))
    new_centroids = []
    for iter in range(max_iter):

        for i in range(req_img.shape[0]):
            #print(i, iter)
            diff = centroids - req_img[i,:]
            d = np.square(diff)
        ...
        for k in range(centroids.shape[0]):
            ...
            if len(indices) == 0:
                continue
            new_centroid = np.mean(req_img[indices,:], axis = 0)
...
def update_image(image, centroids):
    ...
    req_img = np.reshape(image, (image.shape[0] * image.shape[1], 3))
